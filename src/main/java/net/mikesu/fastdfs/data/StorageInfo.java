...
public class StorageInfo {
	...
	public StorageInfo(byte[] data, int offset) {
		...
		offset += AbstractCmd.FDFS_IPADDR_SIZE;version = new String(data, offset, AbstractCmd.FDFS_VERSION_SIZE).trim();
		...
		//新添加connect，统计
		this.connectionAllocCount = (int)AbstractCmd.buff2int(data, offset);
		offset += AbstractCmd.FDFS_PROTO_INT_LEN_SIZE;
		this.connectionCurrentCount = (int)AbstractCmd.buff2int(data, offset);
		offset += AbstractCmd.FDFS_PROTO_INT_LEN_SIZE;
		this.connectionMaxCount = (int)AbstractCmd.buff2int(data, offset);
		offset += AbstractCmd.FDFS_PROTO_INT_LEN_SIZE;
